...
class Agent:
    ...
    def __init__(self,
        ...
        self.critic_opt = torch.optim.Adam(self.critic.parameters(), lr=parser_args.critic_lr)
        self.actor_opt = torch.optim.Adam(self.actor.parameters(), lr=parser_args.actor_lr)

    def run(self) -> None:
        """
        A worker repeatedly samples trajectories from the environment (sample) and loads them in a shard buffer.
